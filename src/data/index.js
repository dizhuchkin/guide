...
export const questionData = new Map([
	...
	[
		...
		[
			...
			{
				id: 66,
				text: "Ключевые слова var, let и const используются для объявления переменных в JavaScript, но имеют различия в области видимости и возможности переназначения и переопределения значений. Область видимости переменной, объявленной с использованием var, ограничена ближайшей функцией или глобальной областью видимости, если переменная объявлена вне функции. Переменные, объявленные с использованием let и const, имеют блочную область видимости, они доступны только внутри блока, в котором были объявлены. Переменные, объявленные с использованием var и let, могут быть переопределены и переназначены. Переменные, объявленные с использованием const, не могут быть переопределены, то есть их значение не может быть изменено после присвоения. Однако объекты и массивы, объявленные с использованием const, могут изменять свои свойства и элементы соответственно.",
			},
			{
				id: 67,
